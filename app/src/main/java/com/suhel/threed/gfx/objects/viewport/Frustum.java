...
public class Frustum extends ViewPort {

    private float left, top, right, bottom, near, far;

    private int projectionMatrixHandle;

    public Frustum(float left, float top, float right, float bottom, float near, float far) {
        this.left = left;
        this.top = top;
        this.right = right;
        this.bottom = bottom;
    ...
    @Override
    public void prepare() {
        Matrix.frustumM(getMatrix().data, 0, getLeft(), getRight(), getBottom(), getTop(),
                near, far);
    }
